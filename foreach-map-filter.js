...
function showFirstAndLast(arr){
    let firstAndLastChars = [];
    arr.forEach(function(value) {
        let len = value.length;
        let first = value[0];
        let last = value[len-1];
        firstAndLastChars.push(first + last);
    });
...
function vowelCount(str){
   ...
   arr.forEach(function(letter) {
    ...
    let vowelIndex = "aeiou".indexOf(lowerCaseLetter);
    if (vowelIndex !== -1) {
...
function removeVowels(str) {
    ...
    let result = strSplit.filter(function(value) {
...
function doubleOddNumbers(arr) {
    ...
    let result = oddNumbers.map(function(number) {
        return number * 2;
    });
    return result;
}
